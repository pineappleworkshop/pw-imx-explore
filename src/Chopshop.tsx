...
const Chopshop = ({ client, wallet }: InventoryProps) => {
  ...
  const [tireInventory, setTireInventory] =
    useState<ImmutableMethodResults.ImmutableGetAssetsResult>(Object)
  const [carSelected, setCarSelected] =
    useState<ImmutableMethodResults.ImmutableAsset>(Object)
  ...
  const burn_mock_address: string =
    process.env.REACT_APP_MOCK_BURN_ADDRESS ?? ''
  ...
  return (
    <Container>
      ...
      <Typography
        sx={{ fontFamily: 'Alegreya Sans SC', fontSize: '2rem', color: 'red' }}
      >
      ...
      <Stack direction="row" sx={{ justifyContent: 'space-around' }}>
        ...
        {tireInventory?.result?.length > 3 && isCarSelected && (
          <Button
            variant="contained"
            size="large"
            sx={{
      ...
      {!isCarSelected && (
        <CarNftList nfts={inventory?.result} onSelect={carSelectedHandler} />
      )}
      {isCarSelected && (
        <NftCarCard
          id={carSelected?.token_id}
          image={carSelected?.image_url}
          nft={carSelected}
          onSelect={carSelectedHandler}
        />
      )}
      <Typography
        sx={{ fontFamily: 'Alegreya Sans SC', fontSize: '2rem', color: 'cyan' }}
      >
        Your Monster Tires:
      </Typography>
